...
//
// ASCOM Switch driver for Web_IO
//
...

{
    ...
    //
    // Your driver's DeviceID is ASCOM.Web_IO.Switch
    //
    // The COM Guid attribute sets the CLSID for ASCOM.Web_IO.Switch
    // The COM ClassInterface/None attribute prevents an empty interface called _Web_IO from being created and used as the [default] interface
    //
    ...
    /// </summary>
    [ComVisible(true)]
    [Guid("ea037e9e-5b07-4cfe-bb5e-45f6921b24ee")]
    [ProgId("ASCOM.Web_IO.Switch")]
    [ServedClassName("ASCOM Switch Driver for Web_IO")] // Driver description that appears in the Chooser, customise as required
    ...
    public class Switch : ReferenceCountedObjectBase, ISwitchV2, IDisposable
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="Web_IO"/> class. Must be public to successfully register for COM.
        /// </summary>
        public Switch()
        {
            try
            {
                ...
                tl = new TraceLogger("", "Web_IO.Driver"); // Remove the leading ASCOM. from the ProgId because this will be added back by TraceLogger.
                SetTraceState();

                // Initialise the hardware if required
                SwitchHardware.InitializeHardware();
